...
help:
	...
	@printf "\tTARGET: Specify the target of which to run menuconfig. Default: x86_64_mender\n"
	@printf "\n"
	@printf "Targets:\n"
	...
	@printf "\tmenuconfig: run menuconfig on a given target. Default: x86_64_mender\n"
...
menuconfig:
	...
	@docker exec -it buildroot-mender /bin/bash -c 'cd ~/buildroot/buildroot-external-mender/output/${TARGET} && make menuconfig'
...
.PHONY: shell
shell:
	@/bin/bash -c 'if [ -z $(docker container ps -aqf "name=buildroot-mender") ]; then make APPLY_CONFIGS=${APPLY_CONFIGS} NO_BUILD=true EXIT_AFTER_BUILD=false upd; fi;'
	@/bin/bash -c 'if [[ ${APPLY_CONFIGS} == "true" ]]; then sleep 5; fi'
	@docker exec -it buildroot-mender /bin/bash
...
.PHONY: x64-run
x64-run:
	@/bin/bash buildroot-external-mender/board/x86_64/qemu-run.sh
